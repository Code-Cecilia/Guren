...
class Levels (commands.Cog):
    ...
    def __init__(self, bot):
        ...
        with open(r"/root/bots/Guren/cogs/users.json", "r") as f:
            self.users = json.load(f)
    
    async def save_users(self):
        await self.bot.wait_until_ready()
        while not self.bot.is_closed():
            with open(r"/root/bots/Guren/cogs/users.json", "w") as f:
    ...
    async def on_message(self, message, guild: discord.Guild = None):
        ...
        if message.author.bot == True:
            return
        if message.channel.id in [556116525128613888, 656880750142160926, 696804345311657995, 612675164227764254, 676698027028905999, 694525461639725070, 694525883129528410]:
            return
        ...
        if self.lvl_up(author_id):
            ...
            embed.set_author(name=f"LEVELUP", icon_url="https://cdn.discordapp.com/attachments/717128065653932053/718842463250284605/ezgif.com-resize_1.gif")

            embed.add_field(name=f"{message.author} ranked up to", value=f"Level: `{self.users[author_id]['level']}`")
            
            embed.set_footer(text=f"Leveling system by Guren Ichinose#6762", icon_url="https://cdn.discordapp.com/avatars/669973381067571240/350b378596401f453fb4d5bd3411a682.webp?size=1024")
