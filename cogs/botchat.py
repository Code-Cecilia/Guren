...
class BotChat(commands.Cog, description='A Cog to... chat with the bot, i guess?\n'
    ...
    async def set_botchat_channel(self, ctx, channel: discord.TextChannel):
        ...
        if not os.path.exists(f'bot_config/guild{ctx.guild.id}.json'):  # create file if not exists
    ...
    async def on_message(self, message):
        ...
        if not os.path.exists(f'bot_config/guild{message.guild.id}.json'):  # create file if not exists
